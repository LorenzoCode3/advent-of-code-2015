...
# Iterate over all combinations and check if the player wins
for combination in combinations:
    player_stats["damage"] = combination[1]
    player_stats["armor"] = combination[2]
    if fight(player_stats.copy(), boss_stats.copy()) == "Player":
        winning_combinations.append(combination)

# Find the minimum expensive combination
print(min(winning_combinations, key=lambda x: x[0])[0])